    ...
    while first_iteration or p.cfg.update_cfg():
        ...
        for defect_type in list(defect_types):
            ...
            if defect_type in (["Violated-AC-Check"]):
                ...
                ac_checks_with_sloads, slices, imap, other_ac_cehcks = (
                    ...
                    if (
                        ...
                        if tainted_sstores.keys() & set(
                            ...
                            other_tainted_santizers_sstores = [
                                ...
                                and sstore_fun_sig
                                != cinfo.get_function_sig(
                                    p.cfg, tainted_sstores[ins], "id"
                                )
                            ]
                    ...
                    for sstore_ins in current_ac_check_slots_sstores:
                        if sstore_ins in checked_tainted_sstores:
                            ...
                            logging.debug(
                                "Path: %s",
                                "->".join(
                                    "%x" % p for p in tainted_sstores[sstore_ins]
                                ),
                            )
                            if not reported:
                                print(
                            ...
                            if mode == "FIntendedB":
                                ...
                                if len(ac_checks_in_sstore_path) == 0:
                                    taint_valid_results = validate_path(
                                        ...
                                        mode=mode,
                                        ac_jumpi=[
                                            s.addr for s in ac_checks_with_sloads
                                        ],
                                    )
                                if taint_valid_results:
                ...
                for ins_type in ins_types:
                    if ins_type == "acsbl_sdestruct":  # accessible selfdestruct
                        ...
                        sinks = {
                            s.addr: opcodes.CRITICAL_ARGS[ins_type] for s in sub_ins
                        }
                        restricted = True
                    if taintedBy == []:
                        taintedBy = opcodes.potentially_user_controlled
                    ...
                    for (
                        ...
                        if (
                            ins_type in (["SELFDESTRUCT", "DELEGATECALL"])
                            and j_r._tainted
                        ):
                    ...
                    for s in tainted_ins:
                        ...
                        if (
                            ...
                            logging.debug(
                                "Path: %s", "->".join("%x" % p for p in j_path)
                            )
                            print(
...
def main():
    ...
    parser.add_argument("-v", "--vul", help="supported defect type")

    parser.add_argument("-t", "--end", help="supported termination type")
    ...
    if not args.bytecode:
        ...
        analysis(
            p,
            initial_storage,
            sym_validation=symbolic_validation,
            mode=mode,
